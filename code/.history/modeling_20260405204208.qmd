...
# Keep only rows with variables needed for the model
model_vars = [
    "escalation",
    "violence_count_lag1",
    "protest_growth",
...
dat = df[model_vars].dropna().copy()

# Train/test split (adjust years if needed to match your paper)
# This assumes year_month is available in mergedcleaned.csv
if "year_month" in df.columns:
    dat = df[["year_month"] + model_vars].dropna().copy()
    dat["year_month"] = pd.PeriodIndex(dat["year_month"], freq="M")
    train = dat[dat["year_month"] <= pd.Period("2022-12", freq="M")].copy()
else:
    train = dat.copy()
...
# Optional: nicer labels for the paper
label_map = {
    "violence_count_lag1": "violence_count_lag1",
    "protest_growth": "protest_growth",
    "democracy_index": "democracy_index",
    "inflation_lag1": "inflation_lag1"
}
coef_table["Variable"] = coef_table["Variable"].map(label_map)
...
coef_table["Coefficient"] = coef_table["Coefficient"].round(2)
coef_table["Interpretation"] = coef_table["Coefficient"].apply(interpret_coef)

# Sort by absolute magnitude if you want
