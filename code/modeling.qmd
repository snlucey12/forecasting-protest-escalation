...
df["escalation"] = (df["violence_count_t1"] >= 1.25 * df["violence_count"]).astype(int)
...
# 1) Grid definition (0.5° ≈ ~50km; can try .25)
grid_size = 0.5
...
# 2) Cell-month aggregation using event flags
df["year_month"] = df["event_date"].dt.to_period("M")

cell_month = (
    df.groupby(["cell_id", "country", "year_month"], as_index=False)
      .agg(
...
# Escalation: violence appears next month in the SAME cell (conditional on protests)
...
# restrict to protest months (matches your country-month design)
cell_month = cell_month[cell_month["protest_any"] == 1].copy()

# Time fixed effects (absorbs common shocks)
cell_month["year"] = cell_month["year_month"].dt.year.astype(int)

# Inference-focused logistic regression with clustered SEs by spatial unit
formula = "escalation ~ protest_count + violence_count + C(year)"

logit_cell = smf.logit(formula, data=cell_month).fit(
    cov_type="cluster",
...
# 1) Load Natural Earth countries (built-in via URL)
...
if "risk_hat" not in cm_plot.columns:
    cm_plot["risk_hat"] = logit_cell.predict(cm_plot)

cm_plot = cm_plot.dropna(subset=["risk_hat", "lat_grid", "lon_grid"])
...
plt.tight_layout()
plt.savefig("risk_map_with_country_borders.png", bbox_inches="tight")
plt.show()

# Save to output folder
OUTDIR = "../output"
plt.savefig(
    os.path.join(OUTDIR, "risk_map_gridcell.png"),
    dpi=300,
    bbox_inches="tight"
)
```
