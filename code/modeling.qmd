...
# One-hot encode region for sklearn (do this BEFORE train/valid/test split if possible)
df = pd.get_dummies(df, columns=["region"], drop_first=True)
region_dummies = [c for c in df.columns if c.startswith("region_")]

# Append extras + region fixed effects
features = features + extra_features + region_dummies

# Final feature list for sklearn
dm_features = [f"{c}_dm" for c in base_features]  # whatever you already do
final_features = dm_features + extra_features

# One-hot encode region for sklearn
df = pd.get_dummies(df, columns=["region"], drop_first=True)
region_dummies = [c for c in df.columns if c.startswith("region_")]

final_features = final_features + region_dummies
...
logit_inference = smf.logit(
    "escalation ~ violence_lag1 + violence_lag2 + violence_rolling3 \
